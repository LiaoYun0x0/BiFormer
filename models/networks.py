...
from models.transformer import LocalFeatureTransformer,GlobalFeatureTransformer,PositionEncodingSine,LocalFeatureTransformerAndChannel
import sys
import math
...
class GLNet(torch.nn.Module):
    def __init__(self,config=cfg,backbone="resnet18"):
        ...
        if backbone == "resnet50":
            ...
            self.feature_conv32 = nn.Conv2d(2048,config["global_feature_dim"],1,1,0,bias=False)
        ...
        # self.local_transformer = LocalFeatureTransformerAndChannel(config["lo_cfg"])
    ...
    def attention_local_feature(self,x0,x1):
        ...
        # x0,x1 = self.local_transformer(x0,x1,size)
        x0,x1 = self.local_transformer(x0,x1)
        return x0,x1

    def forward_pair_lo(self,pbatch,abatch):
        _,lf0_8x,lf0,gf0, lf0_16x = self.backbone.extract_endpoints(pbatch)
        _,lf1_8x,lf1,gf1, lf1_16x = self.backbone.extract_endpoints(abatch)
        ...
        return lf0,lf1, lf0_16x, lf1_16x
