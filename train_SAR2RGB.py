...
from common.functions_old import *
...
def main(args):
    ...
    for epoch in range(args.train_epoch):
        ...
        if log_stats['test_losses'] < best_loss or log_stats['test_losses'] < 3500:
            best_loss = log_stats['test_losses']
            fine_loss = log_stats['test_fine_loss']
            torch.save({'model': model_without_ddp.state_dict()}, f'{save_path}/{epoch}_model_SAR2RGB_rotate_best_mean_std_mish_lbl_{best_loss:.1f}_{fine_loss:.1f}.pth')
        with open(f'{save_path}/train_SAR2RGB_rotate_mean_std_mish_lbl.log', 'a') as f:
